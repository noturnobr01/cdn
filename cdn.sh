...
if [ "$DISTRIB_RELEASE" == "16.04" ]; then
    ...
    echo "Kernel version sufficient, no need to install anything"
elif [ "$DISTRIB_RELEASE" == "20.04" ]; then
    echo "Ubuntu 20.04 detected. Kernel is already sufficient."
else
    # check kernel version
    if dpkg --compare-versions "$KERNEL_VERSION" "ge" "5.4"; then
        echo "WARNING: Non-LTS versions are not supported. Continuing since you have a compatible kernel."
    else
        echo "ERROR: Kernel auto install on Non-LTS versions is not supported. Please manually install kernel >= 5.4."
        exit 1
    fi
fi
